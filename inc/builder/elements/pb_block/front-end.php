    ...
    <div class="penci-header-block penci-header-block-1">
        ...
        if ( $block_id ) {
			if ( did_action( 'elementor/loaded' ) && Elementor\Plugin::instance()->db->is_built_with_elementor( $block_id ) ) {
				...
				echo Elementor\Plugin::$instance->frontend->get_builder_content_for_display( $block_id, $inline_css );

				if ( $inline_css ) {
					wp_deregister_style( 'elementor-post-' . $block_id );
					wp_dequeue_style( 'elementor-post-' . $block_id );
				}
