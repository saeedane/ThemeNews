...
if ( ! function_exists( 'penci_get_builder_mod' ) ) {
	function penci_get_builder_mod( $setting, $default = '' ) {
		...
		} elseif ( is_singular() && ! is_page() && $page_posts ) {
...
add_action( 'penci_footer_builder_content', function () {
	...
	} elseif ( is_singular() && ! is_page() && $footer_post ) {
	...
	if ( $footer_id ) {
		...
		if ( $footer_content ) {
			if ( did_action( 'elementor/loaded' ) && Plugin::$instance->documents->get( $footer_id )->is_built_with_elementor() ) {
				...
				$content .= Plugin::$instance->frontend->get_builder_content_for_display( $footer_id, $inline_css );

				if ( $inline_css ) {
					wp_deregister_style( 'elementor-post-' . $footer_id );
					wp_dequeue_style( 'elementor-post-' . $footer_id );
				}
				$content .= '</div>';
			} else {
				$content .= '<div class="pcfb-wrapper">';
