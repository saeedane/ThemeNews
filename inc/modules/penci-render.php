		...
		if ( $penci_menu_bgimg || $penci_menu_bgcl || $penci_menu_mh || $penci_menu_mw ) {
			...
			if ( $penci_menu_mh ) {
				...
				$css_out       .= '#navigation .menu li.penci-mega-menu.penci-block-wrap-mega-' . $menuid . ' .penci-dropdown-menu,
							.navigation .menu li.penci-mega-menu.penci-block-wrap-mega-' . $menuid . ' .penci-dropdown-menu{min-height:' . $penci_menu_mh . ';display:block;}';
			}
			if ( $penci_menu_mw ) {
			...
			if ( 'yes' !== $penci_menu_load ) {
				...
				if ( $mega_block_content ) {
					...
					if ( did_action( 'elementor/loaded' ) && Plugin::$instance->documents->get( $mega_block_id )->is_built_with_elementor() ) {
						...
						$content .= Plugin::$instance->frontend->get_builder_content_for_display( $mega_block_id, $inline_css );

						if ( $inline_css ) {
							wp_deregister_style( 'elementor-post-' . $mega_block_id );
							wp_dequeue_style( 'elementor-post-' . $mega_block_id );
						}
					} else {
